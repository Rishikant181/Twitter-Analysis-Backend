...
export class UserService {
	...
	async followers(id: string, args: UserListArgsDto): Promise<CursoredDataDto<UserDto>> {
		...
		const followers: CursoredDataDto<UserDto> = new CursoredDataDto<UserDto>([], args.cursor);
		...
		do {
			...
			const data = await this.twitter.api(true).users.getUserFollowers(id, batchSize, followers.next.value);
	...
	async following(id: string, args: UserListArgsDto): Promise<CursoredDataDto<UserDto>> {
		/** The cursored data to be returned. */
		const following: CursoredDataDto<UserDto> = new CursoredDataDto<UserDto>([], args.cursor);
		...
		do {
			...
			// Fetching a single batch
			const data = await this.twitter.api(true).users.getUserFollowing(id, batchSize, following.next.value);
	...
	public async likes(id: string, args: UserListArgsDto): Promise<CursoredDataDto<TweetDto>> {
		/** The cursored data to be returned. */
		const likes: CursoredDataDto<TweetDto> = new CursoredDataDto<TweetDto>([], args.cursor);
		...
		do {
			...
			// Fetching a single batch
			const data = await this.twitter.api(true).users.getUserLikes(id, batchSize, likes.next.value);
